...
export function NewProjectModal({ onClose, onCreate }: { onClose: () => void; onCreate: (p: { title: string; description: string; status: ProjectStatus; dueDate: string }) => void }) {
  ...
  const [due, setDue] = useState("2026-11-15");
  return (
...
export function NewTaskModal({ projects, defaultProjectId, onClose, onCreate }: { projects: Project[]; defaultProjectId?: string; onClose: () => void; onCreate: (t: { projectId: string; title: string; description: string; status: TaskStatus; priority: TaskPriority; dueDate: string }) => void }) {
  ...
  return (
    <Shell onClose={onClose} title="New task" sub="Tasks belong to a project and one owner.">
      <form onSubmit={(e) => { e.preventDefault(); if (title.trim().length < 3 || !projectId) return; onCreate({ projectId, title: title.trim(), description: desc.trim() || "No details yet.", status, priority, dueDate: due }); }} className="space-y-3">
        ...
        <div className="grid grid-cols-3 gap-3">
          <label className="block text-[13px] font-medium text-slate-700">Status<select value={status} onChange={(e) => setStatus(e.target.value as TaskStatus)} className="mt-1 w-full rounded-lg border border-slate-200 px-2 py-2 text-sm"><option value="todo">To do</option><option value="in-progress">In progress</option><option value="done">Done</option></select></label>
...
export function ShortcutsModal({ onClose }: { onClose: () => void }) {
  const rows = [["/", "Focus search"], ["N", "New task"], ["P", "New project"], ["1 / 2 / 3", "Go to Dashboard / Projects / Tasks"], ["Esc", "Close dialogs"] ];
  return (
