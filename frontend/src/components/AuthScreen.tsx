...
export function AuthScreen() {
  ...
  const submit = async (e: React.FormEvent, preset?: { email: string; password: string; name?: string }) => {
    e.preventDefault();
    setError(null);
    setBusy(true);
    try {
      ...
      <div className="flex flex-1 items-center justify-center p-5 sm:p-10">
        <div className="w-full max-w-sm">
          ...
          <form onSubmit={submit} className="mt-4 space-y-3">
            {mode === "register" && (
              <label className="block text-[13px] font-medium text-slate-700">
                Full name
                <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Aarav Mehta" autoComplete="name"
          ...
          <div className="mt-4 rounded-xl border border-slate-200 bg-white p-3.5">
            <p className="text-[13px] font-semibold text-slate-800">Just looking around?</p>
            <p className="mt-0.5 text-xs text-slate-500">One click signs you into a seeded demo workspace.</p>
            <button onClick={(e) => void submit(e, { email: DEMO_EMAIL, password: DEMO_PASSWORD })} disabled={busy}
              className="mt-2.5 w-full rounded-lg border border-slate-200 py-2 text-[13px] font-semibold text-slate-700 transition hover:bg-slate-50 disabled:opacity-60">
              {busy ? "Signing in…" : "Try the demo account"}
            </button>
          </div>
        </div>
      </div>
