...
export function ProjectCard({ project, taskCount, onOpen, compact = false }: { project: Project; taskCount: number; onOpen: () => void; compact?: boolean }) {
  return (
      ...
      <div className="mt-3 flex items-center justify-between border-t border-slate-100 pt-2.5 text-xs text-slate-500">
        <span className="inline-flex items-center gap-1"><CalendarDays size={13} className="text-slate-400" /> {project.dueDate}</span>
          ...
          {project.members.slice(0, 4).map((m) => (
            <span key={m} title={userById(m).name} className="flex h-6 w-6 items-center justify-center rounded-full border-2 border-white bg-slate-700 text-[9px] font-bold text-white">
              {userById(m).avatar}
            </span>
          ))}
        </span>
